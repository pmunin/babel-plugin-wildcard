...
>>>>>>> Checking in minified distribution.
...
var alreadyInitialized={};for(var i=node.specifiers.length-1;i>=0;i--){dec=node.specifiers[i];if(t.isImportNamespaceSpecifier(dec)&&_fs3.default.existsSync(dir)&&!_fs3.default.statSync(dir).isFile()){addWildcard=true;wildcardName=node.specifiers[i].local.name;node.specifiers.splice(i,1)}// This handles { A, B, C } from 'C/*'
...
try{files=(0,_recursiveReaddirSync2.default)(dir).map(function(file){return file.replace(dir+'/','')})// Handle shallow dependencies
<<<<<<< HEAD
.map(function(file){return file.replace(dir,'')}).filter(function(file){return exts.indexOf(_path3.default.extname(file).substring(1))>-1})}catch(e){console.warn('Wildcard for '+name+' points at '+src+' which is not a directory.');return}// This is quite a mess but it essentially formats the file
=======
.map(function(file){return file.replace(dir,'')}).filter(function(file){var _path$parse=_path3.default.parse(file),name=_path$parse.name,ext=_path$parse.ext;return exts.indexOf(ext.substring(1))>-1&&filenameRegex.test(name)})}catch(e){console.warn('Wildcard for '+name+' points at '+src+' which is not a directory.');return}// This is quite a mess but it essentially formats the file
>>>>>>> Checking in minified distribution.
...
.map(function(part){return getFancyName(part,state.opts)})// Now we're 100% settled on the fancyName, if the user
...
var name;if(state.opts.nostrip!==true){name='./'+_path3.default.join(src,_path3.default.dirname(file),_path3.default.basename(file))}else{name='./'+_path3.default.join(src,file)}// Special behavior if 'filterNames'
...
var importDeclaration=t.importDeclaration([t.importDefaultSpecifier(id)],t.stringLiteral(name));// Initialize the top level directories as an empty objects
var nested=parts.slice(0,-1);nested.reduce(function(prev,curr){if(!prev){if(alreadyInitialized[curr])return{path:curr,member:alreadyInitialized[curr]};var _member=t.memberExpression(t.identifier(wildcardName),t.stringLiteral(curr),true);var _setup=t.expressionStatement(t.assignmentExpression('=',_member,t.objectExpression([])));path.insertBefore(_setup);alreadyInitialized[curr]=_member;return{path:curr,member:_member}}var newPath=prev.path+'.'+curr;if(alreadyInitialized[newPath])return{path:newPath,member:alreadyInitialized[newPath]};var member=t.memberExpression(prev.member,t.stringLiteral(curr),true);var setup=t.expressionStatement(t.assignmentExpression('=',member,t.objectExpression([])));path.insertBefore(setup);alreadyInitialized[newPath]=member;return{path:newPath,member:member}},null);// Chain the parts for access
var accessPoint=parts.reduce(function(prev,curr){if(!prev)return t.memberExpression(t.identifier(wildcardName),t.stringLiteral(curr),true);return t.memberExpression(prev,t.stringLiteral(curr),true)},null);// Assign the file to the parts
var setter=t.expressionStatement(t.assignmentExpression('=',accessPoint,id));path.insertAfter(setter);path.insertAfter(importDeclaration)}if(path.node.specifiers.length===0){path.remove()}}}}}};var _path2=require('path');var _path3=_interopRequireDefault(_path2);var _fs2=require('fs');var _fs3=_interopRequireDefault(_fs2);var _recursiveReaddirSync=require('recursive-readdir-sync');var _recursiveReaddirSync2=_interopRequireDefault(_recursiveReaddirSync);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}function getFancyName(originalName,opts){// Strip extension
